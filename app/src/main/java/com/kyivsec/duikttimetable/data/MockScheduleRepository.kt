...
class MockScheduleRepository(loader: MockDataLoader, private val clock: Clock = Clock.systemDefaultZone()) :
    ...
    override fun observeSchedule(owner: TimetableOwner, range: DateRange): Flow<List<ScheduleDay>> = days.map { values ->
        values.filter { it.date in range.startInclusive..range.endInclusive }
    }
    override fun observeCachedDates(owner: TimetableOwner, range: DateRange): Flow<Set<LocalDate>> = days.map { values ->
